...
// 1. Update type to Promise<{ id: string }>
export async function GET(
  ...
  try {
    ...
    const order = await prisma.order.findFirst({
      where: { id: orderId, sellerId: sellerId },
      include: {
        buyer: {
          select: {
            id: true,
          ...
          select: {
            ...
            attributes: {
              include: {
                ...
                value: { select: { value: true, displayName: true, hexColor: true } },
              },
            },
            images: {
              select: { url: true, altText: true, isPrimary: true },
    ...
    const orderDetails = {
      ...
      variantAttributes: order.variant?.attributes.reduce((acc: any, attr: any) => {
...
// Update type to Promise<{ id: string }>
...
export async function PUT(
  ...
  try {
    ...
    // ============================================
    // RESTRICTION: PREVENT PROCESSING -> SHIPPED/DELIVERED
    // ============================================
    
    // Define allowed status transitions
    const allowedTransitions: Record<string, string[]> = {
      'PENDING': ['PROCESSING', 'CANCELLED'],
      // If status is PROCESSING, only allow CANCELLED.
      // Restrict moving to SHIPPED or DELIVERED.
    ...
    if (data.status) {
      ...
      // STRICT CHECK: Validate transition from current status
      if (data.status !== existingOrder.status) {
        const allowedNextSteps = allowedTransitions[existingOrder.status] || [];
        
        if (!allowedNextSteps.includes(data.status)) {
           return NextResponse.json(
             { 
               error: `Cannot change status from ${existingOrder.status} to ${data.status}. Current status allows: ${allowedNextSteps.join(', ')}`
             },
             { status: 400 }
           );
        }
    ...
    if (data.status === 'DELIVERED' && existingOrder.variantId) {
      await prisma.productVariant.update({
        ...
        data: { soldQuantity: { increment: existingOrder.quantity } },
      });
    }
    ...
    if (data.status === 'CANCELLED' && existingOrder.variantId && existingOrder.status !== 'CANCELLED') {
      await prisma.productVariant.update({
        where: { id: existingOrder.variantId },
        data: { currentStock: { increment: existingOrder.quantity } },
      });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!order) {
      return NextResponse.json({ error: 'Order not found or you do not have permission to delete it' }, { status: 404 });
    }
    ...
    await prisma.order.delete({ where: { id: orderId, sellerId: sellerId } });
